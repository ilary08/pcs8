...
def find_superstring(strings): #we have a superstring that is the union all the smaller string
    temp = strings
    ...
    while len(temp) > 1:
        ...
        for i in range(len(temp)-1):
            for j in range(i+1, len(temp)):
                combine_string, overlap_string = overlap_strings(temp[i], temp[j])
