from Bio import SeqIO
data = 'rosalind_grph.txt'
n = 3
seq_name, seq_string = [], [] # I put two [] in order to have enough values to unpack
with open('rosalind_grph.txt', 'r') as file:
    for seq_record in SeqIO.parse(file,'fasta'):
        seq_name.append(str(seq_record.name))
        seq_string.append(str(seq_record.seq))
for i in range(len(seq_string)):
    for j in range(len(seq_string)):
        if i != j: # I needed to compare the suffix of all the sequences to the prefixes and they must be different
            if seq_string[i][-n:] == seq_string[j][:n]:
                print(seq_name[i], seq_name[j])

            

